...
The `/commit` skill is a Claude Code built-in that provides a structured commit workflow. It is not part of Kombuse itself — agents running on other backends do not have access to it.
...
- [Using Kombuse from Claude Code](/guides/claude-code) — set up the MCP connection that enables agent-to-git interaction
