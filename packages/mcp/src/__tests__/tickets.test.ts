...
describe('get_ticket', () => {
  it('should return ticket with comments by default and no image sections', async () => {
    ...
    expect(data.ticket.author).toBeUndefined()
    expect(data.ticket.assignee).toBeUndefined()
    expect(data.participants[TEST_USER_ID]).toBeDefined()
    expect(data.participants[TEST_USER_ID].type).toBe('user')
    expect(data.participants[TEST_USER_ID].name).toBe('Test User')
  ...
  it('should include overview participants by default', async () => {
    ...
    const activity = data.overview.comment_activity.find((p: any) => p.author_id === analyzerId)
    expect(activity).toBeDefined()
    expect(activity.comment_count).toBe(1)
    expect(data.participants[analyzerId]).toBeDefined()
    expect(data.participants[analyzerId].type).toBe('agent')
    expect(data.participants[analyzerId].agent_type).toBe('analyzer')
  })
