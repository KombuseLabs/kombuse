...
export interface GlobRendererProps {
  toolUse: SerializedAgentToolUseEvent
  result?: SerializedAgentToolResultEvent
}

export function GlobRenderer({ toolUse, result }: GlobRendererProps) {
  ...
  const fileCount = outputContent ? countOutputLines(outputContent) : 0
  const noMatches = outputContent !== null && fileCount === 0
      ...
      <div className={`rounded-lg text-sm ${isError ? 'bg-red-500/5 ring-1 ring-red-500/20' : 'bg-muted/30'}`}>
        <CollapsibleTrigger className="flex w-full items-center gap-2 px-3 py-2 text-left">
          ...
          <div className="flex flex-col">
            ...
            {isError ? (
              <span className="text-[10px] text-red-600 dark:text-red-400">Glob failed</span>
            ) : outputContent !== null ? (
              <span className="text-[10px] text-muted-foreground">
                {noMatches ? 'No files found' : `${fileCount} file${fileCount === 1 ? '' : 's'} found`}
              </span>
            ) : null}
          </div>
