...
describe('buildCleanPath', () => {
  ...
  it('prepends common dirs in the correct order', () => {
    ...
    expect(parts[2]).toBe('/opt/homebrew/sbin')
    expect(parts[3]).toBe('/opt/local/bin')
    expect(parts[4]).toBe('/Users/testuser/.nix-profile/bin')
    expect(parts[5]).toBe('/nix/var/nix/profiles/default/bin')
    expect(parts[6]).toBe('/usr/local/bin')
    expect(parts[7]).toBe('/usr/bin')
    expect(parts[8]).toBe('/bin')
    expect(parts[9]).toBe('/some/custom/path')
  })
